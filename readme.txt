       ***   FRACT386 (Version 2.1)   ******   by Bert Tyler   ***

This program generates fractals using 32-bit integer arithmetic specific
to the 386 microprocessor.   It will politely refuse to execute if it finds
itself on any earlier Intel processors such as the 8086, 80186 or 80286.
The program will work with CGA, EGA, MCGA, VGA and many popular super-rez
adapters, and can switch display modes on-the-fly for those adapters
capable of multiple emulation.  For instance, I can run this program in
any of the IBM CGA/EGA/MCGA/VGA modes on my PS/2 model 80.   The program
does not perform any floating point arithmetic, and does not require an FPU.
...
effects on its own.
   ...
   Starting with version 2.1, the program accepts optional parameters.
The full FRACT386 command line is actually:

         FRACT386 [ Xmin Xmax Ymin Ymax ]

   The four parameters are floating point values that specify the X and Y
coordinates of the corners of the screen that is initially displayed.
These parameters are the same values (in the same order) that display on
the screen when you hit the TAB key after generating a fractal image.
...
displays that you might want to see again (or show to somebody else),
and the floating point notation is the one commonly used by other
fractal programs, so you can compare pictures with your buddy's MAC-II.
The following examples are from screens that I have played with:
   ...
   Beginning with Version 2.1, FRACT386 uses a Video Adapter Table in
the "C" program to reference everything it needs to know about any
particular adapter/mode combination.   This table can contain information
for up to forty adapter/mode combinations, and is automatically tied to
forty Function Keys (F1-F10 and their Control/Shift/Alt variants) when
the program is running.   This table makes adding support for various
third-party video cards and their high-rez modes much easier, at least for
the ones that pretend to be a standard adapter with more dots and/or colors.   

   Starting with this release, the assembler object code (FRASM386.OBJ)
is included in the ARC files to make it easier for those of you that have
a fancy video adapter and want to modify FRACT386 to support it.   You will
at a minimum need a "C" compiler that supports "Medium" model object code
(Microsoft or Borland will work) and enough information about your adapter
to throw it into one of its unique modes via INT 10.
...
of Tom Moran (tmoran on BIX).   I tried calling up each of the "major" video
...
distributed (on BIX) by Doug Klein as DKMANDEL.ARC.   I kept adding this
and replacing that, until one day I looked around and couldn't find any of
Doug's code lying around anymore, so I guess it's now an original program...
...
got the idea to do same thing on a PS/2 model 80, I first called Dave to
...
that start FRACT386 on a particular piece of the fractal plane -- then
picture that as the ONLY way to select an area to draw).   That's when I
added the zoom-and-pan interface. It has passed a pretty robust ease-of-use
test -- my seven year old likes to play with it.   She wants me to modify
it to work with a mouse, though (sigh).
