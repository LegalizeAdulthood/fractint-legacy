...
singlepass:
	...
	je	short quickkbd		;  when using 16-bit logic,
	mov	kbdcount,500		;  test more often
quickkbd:
...
nokey:
	...
	jne	no32bitcode		;  yup.  slow down.
	cmp	cpu,386			; are we on a 386?
	je	short code32bit		; YAY!! Warp nine, Mr Sulu.
no32bitcode:
	call	near ptr code16bit	; BOO!! nap time.
...
;	mult16bit performs 32bit x 32bit signed multiplies, with
...
mults4:
	...
	mov	si,temp+2		;  ...
	shl	si,1			; shift one bit (works on ANY 80xxx)
	rcl	ax,1			;  ...
	rcl	dx,1			;  ...
...
start16bit:			
	...
	mov	cx,bx			;  ...
	mov	di,si			;  ...
	call	mult16bit		; do it.
	jnc	ok16bit1		; no overflow.  continue.
	jmp	end16bit		; overflow.  we done.
ok16bit1:
	...
	mov	di,si			;  ...
	call	mult16bit		; do it.
	jnc	ok16bit2		; no overflow.  continue.
	jmp	end16bit		; overflow.  we done.
ok16bit2:
	...
	jo	short end16bit		; bail out if too high
	js	short end16bit		;  ...
