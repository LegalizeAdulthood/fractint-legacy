...
int full_color;
/* MRR */
char light_name[80] = "light001.tga";
...
static int num_tris; /* number of triangles output to ray trace file */
...
int line3d(unsigned char pixels[], unsigned linelen)
{
   ...
   if(rowcount++ == 0)
   {
	 ...
	 /* Set origin[] to be the center of the untransformed xy plane
	    at z=0 and torigin[] to be the same */
	 origin[0] = (double)(xdots)/2.0;
	 origin[1] = (double)(ydots)/2.0;
	 origin[2] = 0.0;

	 /* Set length of light direction vector for easy viewing */
  ...
  /**************************************************************************/
   ...
   if(RAY && col == 0 && currow != 0)	 start_object();
   ...
   if(FILLTYPE==0 && !SPHERE && !pot16bit && !RAY && debugflag != 2224 )
	 ...
	 switch(FILLTYPE)
	 {
	    ...
	    case 3: /* no interpolation */
		  ...
		  (*plot)(lastrow[next].x,lastrow[next].y,lastrow[next].color);
	       }
	       break;
	    ...
	    case 6:
		  ...
		  /* if colors out of range, set them to min or
		     max color index */
		  /* but avoid background index. This makes colors
		     "opaque" so  */
		  /* SOMETHING plots. These conditions shouldn't happen but */
		  /* just in case			    */
		  ...
		  /* why "col < 2"? So we have sufficient geometry for
		     the fill */
		  /* algorithm, which needs previous point in same row
		     to have  */
...
loopbottom:
	 ...
	 if (RAY || (FILLTYPE != 0 && FILLTYPE != 4))
	 {
	    ...
	    /* if we're at the end of a row, close the object */
	    {
	       ...
	       if (ferror (File_Ptr1))
	       {
		  static char far msg[] = {"Disk error (full?) writing "
					   "ray trace file, file closed"};
		  fclose (File_Ptr1);
		  stopmsg(0,msg);
		  EXIT_OVLY;
		  return(-1);
...
double *pxmin,*pymin,*pzmin,*pxmax,*pymax,*pzmax;
{
   ...
   *pxmax = *pymax = *pzmax = INT_MIN;
   ...
   b[1][0] = b[2][0] = t[1][0] = t[2][0] = xdots-1;
   b[2][1] = b[3][1] = t[2][1] = t[3][1] = ydots-1;
   t[0][2] = t[1][2] = t[2][2] = t[3][2] = zcoord-1;
   ...
   for (i = 0; i < 4; ++i)
   {
      ...
      if (b[i][0] <= *pxmin) *pxmin = b[i][0];
      if (b[i][0] >= *pxmax) *pxmax = b[i][0];
      if (t[i][0] <= *pxmin) *pxmin = t[i][0];
      if (t[i][0] >= *pxmax) *pxmax = t[i][0];
      if (b[i][1] <= *pymin) *pymin = b[i][1];
      if (b[i][1] >= *pymax) *pymax = b[i][1];
      if (t[i][1] <= *pymin) *pymin = t[i][1];
      if (t[i][1] >= *pymax) *pymax = t[i][1];
      if (b[i][2] <= *pzmin) *pzmin = b[i][2];
      if (b[i][2] >= *pzmax) *pzmax = b[i][2];
      if (t[i][2] <= *pzmin) *pzmin = t[i][2];
      if (t[i][2] >= *pzmax) *pzmax = t[i][2];
   }
   ...
   if(show)
   {
	 ...
	 t[i][1] += yyadjust;
      ...
      /* bottom */
      vdraw_line (b[0],b[1],2);
      vdraw_line (b[1],b[2],2);
      vdraw_line (b[2],b[3],2);
      vdraw_line (b[3],b[0],2);

      /* top */
      vdraw_line (t[0],t[1],3);
      vdraw_line (t[1],t[2],3);
      vdraw_line (t[2],t[3],3);
      vdraw_line (t[3],t[0],3);

      /* sides */
      vdraw_line (b[0],t[0],4); /* these pixels written first - want in BACK */
      vdraw_line (b[1],t[1],5);
      vdraw_line (b[2],t[2],6); /* these pixels written last - want in FRONT */
      vdraw_line (b[3],t[3],7);
   }
}
...
/* transform the corners if necessary */
   ...
   b1[0] = b1[0] * aspect - temp;
   t1[0] = t1[0] * aspect - temp;
   b2[0] = b2[0] * aspect - temp;
   t2[0] = t2[0] * aspect - temp;
   b3[0] = b3[0] * aspect - temp;
   t3[0] = t3[0] * aspect - temp;
   b4[0] = b4[0] * aspect - temp;
   t4[0] = t4[0] * aspect - temp;
...
/* this distance formula is not the usual one - but it has the virtue
   ...
   if(0 <= x	 && x < xdots	&&
       ...
       (transparent[1] == 0 || color > transparent[1] ||
	transparent[0] > color))
   {
      if (full_color)
      {
	 ...
	 targa_writedisk (x+sxoffs, y+syoffs, RGB[0], RGB[1], RGB[2]);
      ...
      if (FILLTYPE >= 5)
	 color = (unsigned)(1 + (color * IAmbient)) / 256;
	 standardplot(x,y,color);
   }
}
...
char *File_Name1;
{
   ...
   if (status == -1)
   {  static char far str1[] ={"OOPS - Couldn't Open  <"};
      sprintf(msgbuf,"%Fs%s >", str1, File_Name1);
      stopmsg(0,msgbuf);
      return(-1);
   }
   ...
   if (status == -2)
   {
      ...
      static char far temp1[] = {"OOPS - Ran out of disk space.\n"
				 "You will need a min of\n"
				 "  < "};
      static char far temp2[] = {"bytes >\n"
				 "of disk space to create < "};
      static char far temp3[] = {" >\nI will continue with screen only."};

      sprintf(msgbuf,"%Fs%ld%Fs%s%Fs",
	  temp1, (3L*(long)xdots*(long)ydots)+18L, temp2,
	  File_Name1, temp3);
      stopmsg(0,msgbuf);
      return(-1);
   }
...
static int Open_T_24(char *File_Name2)
{
   int i,j;
   extern xdots;
   extern ydots;
   ...
   if ((fp=fopen(File_Name2,"w+b"))==NULL)
   ...
   /*  Write image size  */
   fputc((unsigned char)(xdots &  0xff),fp);
   fputc((unsigned char)(xdots >> 8),	fp);
   fputc((unsigned char)(ydots &  0xff),fp);
   fputc((unsigned char)(ydots >> 8),	fp);
   ...
   for (i = 0; i < ydots; i++)	/* "clear the screen" (write to the disk) */
      ...
      {
	 fputc(back_color[2], fp);	     /* Targa order (B, G, R) */
	 fputc(back_color[1], fp);
	 fputc(back_color[0], fp);
      }
   if (ferror (fp))
   {
   ...
   if (targa_startdisk(fp, T_header_24) != 0)
   {
      enddisk();
      remove (light_name);
      return(-3);
...
int i,j;
   ...
   {  /*  Finish up the ray tracing files */
	 ...
	 fprintf (File_Ptr1, "*/\n");
      fclose(File_Ptr1);
      File_Ptr1 = NULL;
      updatesavename(ray_name);
   }

   if (full_color && FILLTYPE >= 5)
   { /*  Finish up full color targa files */
      enddisk();
